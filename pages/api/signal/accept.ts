...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResp>) {
  ...
  if (doc.userId === userId) return res.status(400).json({ error: 'cannot_accept_own' });
  if (doc.canceledAt || doc.expiresAt <= now) return res.status(400).json({ error: 'expired' });
  if (doc.acceptedByUserId && doc.acceptedByUserId !== userId) return res.status(409).json({ error: 'already_accepted' });

  const newExpires = new Date(now.getTime() + 30 * 60 * 1000);

  await signals.updateOne(
    { _id: doc._id, $or: [{ acceptedByUserId: null }, { acceptedByUserId: { $exists: false } }, { acceptedByUserId: userId }] },
    { $set: { acceptedByUserId: userId, acceptedAt: now, expiresAt: newExpires } }
  );

  try {
    const io = getSocketServer((res.socket as any)?.server);
    const room = `toilet:${doc.toiletId}`;
    io.to(room).emit('paper_accepted', { signalId, acceptedByUserId: userId });
